...
class MockAGCCrash extends AGCCrash {
  bool enableCrashCollectionCalled = false;

  @override
  Future<void> enableCrashCollection(bool enabled) async {
    enableCrashCollectionCalled = true;
  }
}
...
void main() {
  testWidgets('Enable Crash Collection Button Test',
    ...
    // Create a mock AGCCrash instance
    final mockAGCCrash = MockAGCCrash();

    // Build the widget under test
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(
          body: MyWidget(agcCrash: mockAGCCrash),
        ),
      ),
    );
    ...
    // Verify that the AGCCrash's enableCrashCollection method is called
    expect(mockAGCCrash.enableCrashCollectionCalled, isTrue);
...
// Enable Crash Collection widget
class MyWidget extends StatefulWidget {
  final AGCCrash agcCrash;

  MyWidget({required this.agcCrash});
...
class MyWidgetState extends State<MyWidget> {
  ...
  void _enableCollection() async {
    await widget.agcCrash.enableCrashCollection(true);
